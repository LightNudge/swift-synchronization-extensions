...
public struct AnyConditionVariable<Lock: Lockable>: ConditionVariableProtocol {
    ...
    private let cvLock = SynchronizationExtensions.Lock()
    ...
    private func wait(
        lock: Lock,
        waitInternal: (ConditionVariable, SynchronizationExtensions.Lock) -> Void
    ) {
        cvLock.lock()
    ...
    private func wait(
        lock: Lock,
        waitInternal: (ConditionVariable, SynchronizationExtensions.Lock) -> Bool
    ) -> Bool {
        cvLock.lock()
        lock.unlock()
